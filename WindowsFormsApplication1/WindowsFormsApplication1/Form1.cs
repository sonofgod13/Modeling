...
namespace WindowsFormsApplication1
{
    public partial class Form1 : Form
    {
        ...
        public Form1()  //при загрузке формы основных параметров
        {
 ...
 * */
        ...
        private void finish_button_Click(object sender, EventArgs e)
        {
            ...
            Dictionary<int, double[]> y = new Dictionary<int, double[]>();
            List<int> days = new List<int>();
            for (int i = 0; i < imitator.getFinishedDemandsPerDayStatistic().Length; i++)
            {
                days.Add(i + 1);
            }
            x[0] = days.ToArray();
            y[0] = imitator.getFinishedDemandsPerDayStatistic();
        ...
        private void front_office_button_Click(object sender, EventArgs e)
        {
            if (frontOfficeFlag == true)  
            {
                ...
                string url = ConfigurationSettings.AppSettings["startFrontOfficeUrl"].ToString(); 
                Process.Start("IExplore.exe", url); 
            }
            else
            {
                string url = ConfigurationSettings.AppSettings["frontOfficeUrl"].ToString();
                Process.Start("IExplore.exe", url); 
            }
        }
